#################################################################
# Code written by Edward Choi (mp2893@gatech.edu), modified by Ruhana Azam
...
def load_data(seqFile, labelFile, timeFile):
	seq = pickle.load(open(seqFile, 'rb'))
	labels = pickle.load(open(labelFile, 'rb'))

	seq, x_test, labels, y_test = train_test_split(seq, labels, test_size=0.2)
	x_test, x_val, y_test, y_val =train_test_split(x_test, y_test, test_size=0.5)

	train_set_x = seq #pickle.load(open(seqFile+'.train', 'rb'))
	valid_set_x = x_val #pickle.load(open(seqFile+'.valid', 'rb'))
	test_set_x = x_test #pickle.load(open(seqFile+'.test', 'rb'))
	train_set_y = labels #pickle.load(open(labelFile+'.train', 'rb'))
	valid_set_y = y_val #pickle.load(open(labelFile+'.valid', 'rb'))
	test_set_y = y_test #pickle.load(open(labelFile+'.test', 'rb'))
	train_set_t = None
	valid_set_t = None
	test_set_t = None
...
def precision_recall(y, y_hat, k=20):
	y = y.reshape(-1, y.shape[-1])
	y_hat = y_hat.reshape(-1, y_hat.shape[-1])

	#get top-k indices
	y_hat_k = np.argpartition(y_hat.T, -k, axis=0)[-k:].T
	...
	for i in range(len(y)):
		...
		if len(y_ki) > 0:
			r_i = (10.0 * len(union)) / len(y_ki) #precision for one example
		else:
			r_i = (10.0 * len(union)) / 1 
		p_i = (10.0 * len(union)) / k #recall for one example
		p_list.append(p_i)
		r_list.append(r_i)

	#page 8 formula [Lee et al 2020]
	precision = (1.0 / len(y)) * sum(p_list) #averaging the precision per example
	recall = (1.0 / len(y)) * sum(r_list) #averaging the recall per example
...
def train_doctorAI(
	...
	batchX = testSet[0]
	batchY = testSet[1]
	x_mat, y_mat, mask_mat, lengths = padMatrixWithoutTime(batchX, batchY, options)

	#perform inference
	y_hat = results.eval({x : x_mat, mask: mask_mat})

	#calculate precision and recall
	precision, recall = precision_recall(y_mat, y_hat, 10)
	precision, recall = precision_recall(y_mat, y_hat, 20)
	precision, recall = precision_recall(y_mat, y_hat, 30)
	return
