...
# Written by Edward Choi (mp2893@gatech.edu) and modified by Ruhana Azam
...
if __name__ == '__main__':
	...
	admDxMap = {}
	admDxMap_3digit = {} #added by Ruhana
	...
	pidSeqMap = {}
	pidSeqMap_3digit = {} #Added by Ruhana

	for pid, admIdList in pidAdmMap.iteritems():
		if len(admIdList) < 2: continue
		sortedList = sorted([(admDateMap[admId], admDxMap[admId]) for admId in admIdList])
		pidSeqMap[pid] = sortedList
		sortedList_3digit = sorted([(admDateMap[admId], admDxMap_3digit[admId]) for admId in admIdList]) #Added by Ruhana
		pidSeqMap_3digit[pid] = sortedList_3digit #Added by Ruhana
	...
	for pid, visits in pidSeqMap_3digit.iteritems():
		seq_temp = []
		for visit in visits:
			seq_temp.append(visit[1])
		seqs_3digit.append(seq_temp)
	####
	...
	print("types: " + str(len(types.keys())))
	print("3-digit types: " + str(len(types_3digit.keys())))
	print("seq len: " + str(len(newSeqs)))
	print("3-digit seq len: " + str(len(seqs_3digit)))
