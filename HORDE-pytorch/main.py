...
for n_itr in range(NUM_ITR):
    ...
    if n_itr % n_printiters ==0:
        if len(device_ids)>1:
            ...
            #print(visitslen_batch)
            recall_20 = metrics.compute_recall_k(tv_labels_test[:, :, :input_num_events], tv_outputs_test[:, :, :input_num_events], visitslen_batch,recall_at, 20)
            recall_30 = metrics.compute_recall_k(tv_labels_test[:, :, :input_num_events], tv_outputs_test[:, :, :input_num_events], visitslen_batch,recall_at, 30)
