...
def train_med2vec(seqFile='seqFile.txt', 
	...
	seqs, x_test, labels, y_test = train_test_split(seqs, labels, test_size=0.2)
	...
	#do precision and recall calulation for test set-----------------
	batchX = x_test
	batchY = y_test
	x_mat, y_mat, mask, iVector, jVector = padMatrix(batchX, batchY, options)
	
	#set up model (again...)
	x = T.matrix('x', dtype=config.floatX)
	emb = T.maximum(T.dot(x, tparams['W_emb']) + tparams['b_emb'],0)
	visit = T.maximum(T.dot(emb, tparams['W_hidden']) + tparams['b_hidden'],0)
	results = T.dot(visit, tparams['W_output']) + tparams['b_output']


	#inference
	y_hat = results.eval({x : x_mat})
